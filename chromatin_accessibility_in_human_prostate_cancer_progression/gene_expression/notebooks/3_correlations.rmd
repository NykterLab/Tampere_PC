...
params:
   ...
   debug: no
   load_env: no
   annotations_folder: reference
   tmp_dir: /scratch/atac_tmp
---
...
tss_gid <- fread("tss_grid/all_genes_500u500d_sorted.bed") # gid is gene id
...
do_correlation_tss <- function (atac_mat, gene_mat, mirna_mat,
                                proteins_mat, samples_blacklist, shuffle,
                                atac_feature_type, cluster_object = NA) {

  message("Computing matrix of correlations for TSS...")
  ...
  genes <- unique(c(rownames(gene_mat), rownames(mirna_mat), rownames(proteins_mat)))
  lookup_table_id <- data.frame(atac_id=rownames(atac_mat)[rownames(atac_mat) %in% genes],
                                gene_id=rownames(atac_mat)[rownames(atac_mat) %in% genes])

  cormat <- do_correlation_core(atac_mat, gene_mat, mirna_mat, proteins_mat,
    lookup_table_id, valid_samples, shuffle, cluster_object)

  message("TSS all done.")
  return(cormat)
}
...
tss_exectime <- microbenchmark({
 tss <- do_correlation_tss(atac_tss_mat, gene_mat, mirna_mat, proteins_mat, samples_blacklist, shuffle = FALSE)
}, times = 1)
...
A bash script to parse DARs bed files and generate features id (chr_start_end_comparison), merge all comparisons and run bedtools intersect.

```{bash do_bedtools_intersect_tad_genes_dars, eval = FALSE}
module load compbio/bedtools

cd dar_and_tad/lncap/with_comparison_group_dars_25072019

ln -s ../../dar/dars_25072019/dars_pc_bph_no_overlap_no_chrM_artifact.bed
ln -s ../../dar/dars_25072019/dars_crpc_pc_no_overlap_no_chrM_artifact.bed
ln -s ../../dar/dars_25072019/dars_crpc_bph_no_overlap_no_chrM_artifact.bed
...
A Python script to generate a lookup table from overlap file (Bedtools)
...
Similar to above, generate a overlap file using Bedtools.
...
Parse the overlap file with a Python script.

```{python do_cartesian_product_peaks, eval = FALSE}
import os
os.chdir("peaks_in_tad")
...
suppTable3_dirname <- file.path(out_dir, "tables", "suppTable3")
...
ggsave(plt, filename=file.path(out_dir, "pictures", "figure3H.pdf"), width = 12, height = 5)
...
tf <- list(pca=read.table("tables/gtrd_counts_pca", stringsAsFactors = FALSE),
           all=read.table("tables/gtrd_counts", stringsAsFactors = FALSE))
tf <- lapply(tf, function(tbl) cbind(tbl, BPH=0, PC=0, CRPC=0))

peaks_and_their_samples <- fread("slopped_summits/unified_peaks_with_sample_type/unified_peaks_with_sample_type.tsv")
...
gtrd_dar_in_tad <- subset(dar_in_tad, (abs(pearson) >= 0.5 | abs(spearman) >= 0.5) & comparison_group %in% c("pc_bph", "crpc_pc"),
...
# export(gr, "ar_peaks.bed", format = "bed")
...
df <- data.frame(chrom=gsub("(chr.+)_[0-9]+_[0-9]+", "\\1", ar_dar$dar_id),
                 ...
                 stringsAsFactors = FALSE)
gr <- makeGRangesFromDataFrame(df, starts.in.df.are.0based = FALSE, keep.extra.columns = TRUE)
# export(gr, "ar_dar.bed", format = "bed")
...
bs_tot <- read.table("tables/gtrd_counts")
...
conmat_filter <- do_panel_A(conmat, "pictures/figure4A.pdf")
pheat <- do_panel_B(conmat_filter, "pictures/figure4B.pdf", clustering_method = panel_B_clustering_method)
do_panel_C(pheat, "pictures/figure4C.svg")

### TEST WITH PEAKS
conmat_peaks_filter <- do_panel_A(conmat_peaks, "pictures/figure4A_peaks.pdf", thr = 1000)
pheat_peaks <- do_panel_B(conmat_peaks_filter, "pictures/figure4B_peaks.pdf", thr = 3000, clustering_method = panel_B_clustering_method)
do_panel_C(pheat_peaks, path = "pictures/figure4C_peaks.svg", target_group = "peaks")

### GET DARs CLUSTERING AND PEAKS GENESET
do_panel_C(pheat, path = "pictures/figure4C_peaks_dars_clusters_upgma.svg", target_group = "peaks")
