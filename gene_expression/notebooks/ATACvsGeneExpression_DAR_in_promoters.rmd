...
for (i in seq_along(dars)) {
  ...
  res <- data.frame(dar_id=d, gene_id=p, pearson=NA, spearman=NA, distance=NA)
  ...
  for (j in seq_along(hits)) {
    ...
    res[j,c("pearson", "spearman", "distance")] <- c(pj,sj,dj)
  ...
  res <- as.data.frame(mergeGtrd(as.data.table(res)))
