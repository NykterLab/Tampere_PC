...
mergeWithTad <- function (dar.tbl, tad, colname = NULL)    {
  ...
  dar.gr <- makeGRangesFromDataFrame(dar.tbl, 
  ...
  dt <- data.table(dar_id = dar.tbl[["dar_id"]][queryHits(olaps)], tad = as.character(tad[subjectHits(olaps)]))
...
mergeGtrd <- function (df, gtrd) {
  ...
  gr    <- makeGRangesFromDataFrame(df, seqnames.field = "Chr", start.field = "Start", end.field = "End",
                                    starts.in.df.are.0based = FALSE, keep.extra.columns = TRUE, 
                                    ignore.strand = TRUE)
  olaps <- findOverlaps(gtrd, gr, type = "within")
  tf_df <- data.table(dar_id = gr$dar_id[subjectHits(olaps)], gtrd_tf = gtrd$name[queryHits(olaps)])
  tf_df <- tf_df[,paste(unique(gtrd_tf), collapse = ", "), by = dar_id]
  names(tf_df) <- c("dar_id", gtrd_colname)
  # tf_df <- aggregate(. ~ dar_id, data = tf_df, FUN = function(x) paste(as.character(unique(tf_df[x, gtrd_colname])), collapse = ", "))
  merge(df, tf_df, by = "dar_id", all.x = TRUE)
}
```
...
for (i in seq_along(target)) {
  ...
  t <- mergeGtrd(t, gtrd_pca)
  t <- mergeGtrd(t, gtrd_all)
  results[[i]] <- t
}
